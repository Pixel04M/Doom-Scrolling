...
class HandDetectionAnalyzer(
    ...
    private fun processHandLandmarks(result: HandLandmarkerResult) {
        try {
            ...
            Log.d("HandDetection", "Gesture detection - 5FingersOpen: $isFiveFingersOpen, 5FingersClosed: $isFiveFingersClosed")
            
            // Get index finger position for scrolling (only if not in pause/unpause gesture)
            val indexFingerPosition = if (!isFiveFingersOpen && !isFiveFingersClosed && isIndexFingerRaised(hand)) {
                val position = IndexFingerPosition(
                    x = hand[8].x(), // Index finger tip X (landmark 8)
                    y = hand[8].y()  // Index finger tip Y
                )
                Log.d("HandDetection", "Index finger position: (${position.x}, ${position.y})")
                position
            } else {
                null
            }
